...
- **Embedded images (PDF):** the PDF pipeline runs with `generate_picture_images=True` and markdown export uses `ImageRefMode.EMBEDDED`, so figures and inline images become base64 `data:` URIs directly in the CLOB. Each document is self-contained — no separate file storage needed. `CONTENT` is sized `CLOB(50M)` to accommodate image-heavy docs.
- **Images (HTML):** docling's HTML pipeline detects images but drops their URLs. We recover them by fetching the page ourselves (via `requests` + `beautifulsoup4`), extracting every `<img src="...">`, resolving relative paths to absolute URLs, and appending markdown image references at the end of the saved content. The browser fetches the bitmaps at view time, so the CLOB stays small but viewing requires internet access. Avatars and icons appear alongside content images; filtering is a later refinement.
