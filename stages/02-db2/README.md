...
- **Embedded images:** the PDF pipeline runs with `generate_picture_images=True` and markdown export uses `ImageRefMode.EMBEDDED`, so figures and inline images become base64 `data:` URIs directly in the CLOB. Each document is self-contained — no separate file storage needed. `CONTENT` is sized `CLOB(50M)` to accommodate image-heavy docs.
